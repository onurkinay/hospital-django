...
def details(request,id): 
    queryset = Administrator.objects.filter(ID=id).values()
...
def edit(request,id=-1):  
    if id == -1:
          id = Administrator.objects.filter(User_id = request.user.id).values()[0]["ID"]
    context ={}
    obj = get_object_or_404(Administrator, ID = id)
...
def delete(request,id):
    if request.method =="POST" and id!=1:
        Administrator.objects.filter(ID=id).update(IsVisible=False)
